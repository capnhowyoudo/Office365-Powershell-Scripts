<#
.SYNOPSIS
Exports Office 365 (Microsoft 365) mailbox hold information to CSV.

.DESCRIPTION
This script connects to Exchange Online and generates one or more reports containing
mailboxes with various types of holds, such as Litigation Hold, In-Place Hold, and
Retention Hold. It supports MFA and non-MFA authentication, automatically installs
Exchange Online PowerShell if missing (with user confirmation), and exports the results
into CSV files with timestamps. The script is scheduler-friendly and supports providing
credentials as parameters.

.EXAMPLE
Example 1:
Export all mailbox holds:

.\Export_Litigation_Hold_Reports.ps1

Example 2:
Export Litigation Hold mailboxes only:

.\Export_Litigation_Hold_Reports.ps1 -LitigationHoldsOnly

Example 3:
Export In-Place Hold mailboxes using stored credentials:

.\Export_Litigation_Hold_Reports.ps1 -Username "admin@example.com" -Password "MySecurePassword" -InPlaceHoldsOnly

Example 4:
Export Retention Hold mailboxes only:

.\Export_Litigation_Hold_Reports.ps1 -RetentionHoldsOnly
#>
