<#
.SYNOPSIS
Exports Office 365 email forwarding configuration to a CSV report.

.DESCRIPTION
This script connects to Exchange Online and exports all types of email forwarding
configurations—including ForwardingSMTPAddress, ForwardingAddress, Inbox Rules,
and Transport Rules—into CSV format. It supports MFA and non-MFA authentication
and can be run manually or scheduled with credentials.

This script requires the **ExchangeOnlineManagement** module to connect to
Exchange Online.

.NOTES
Parameter Examples:

1. Run script with MFA prompt:
    .\Export_Email_Forwarding_Report.ps1

2. Run script using non-MFA credentials:
     .\Export_Email_Forwarding_Report.ps1 -UserName "admin@example.com" -Password "MyPass123"

3. Export Inbox rule forwarding only:
     .\Export_Email_Forwarding_Report.ps1 -InboxRules

4. Export Mail Flow (Transport) rule forwarding only:
     .\Export_Email_Forwarding_Report.ps1 -MailFlowRules

5. Run with stored credentials for automation:
     .\Export_Email_Forwarding_Report.ps1 -UserName "svc-automation@example.com" -Password "ServicePass123"
